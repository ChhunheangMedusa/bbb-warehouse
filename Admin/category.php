...
$sort_option = isset($_GET['sort']) ? sanitizeInput($_GET['sort']) : 'name_asc';
...
if (!array_key_exists($sort_option, $sort_mappings)) {
    $sort_option = 'name_asc';
}

$sort_field = $sort_mappings[$sort_option]['field'];
$sort_direction = $sort_mappings[$sort_option]['direction'];
...
// Build query for categories with sorting and search
$query = "SELECT * FROM categories $search_condition ORDER BY $sort_field $sort_direction LIMIT :limit OFFSET :offset";
        ...
        <div class="card-body">
            <?php if (empty($categories)): ?>
                <div class="alert alert-info">
                    <?php if (!empty($search_term)): ?>
                        <?php echo t('no_categories_found_for_search'); ?> "<?php echo htmlspecialchars($search_term); ?>"
                    <?php else: ?>
                        <?php echo t('no_categories'); ?>
                    <?php endif; ?>
                </div>
            <?php else: ?>
