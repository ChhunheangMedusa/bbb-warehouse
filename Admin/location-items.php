...
$sort_mapping = [
    ...
    'date_asc' => ['field' => 'i.created_at', 'direction' => 'ASC'],
    'date_desc' => ['field' => 'i.created_at', 'direction' => 'DESC']
];
...
if ($month_filter && $month_filter !== 'all') {
    $query .= " AND MONTH(i.created_at) = :month";
    $count_query .= " AND MONTH(i.created_at) = :month";
    $params[':month'] = $month_filter;
}

// Add year filter condition
if ($year_filter && $year_filter !== 'all') {
    $query .= " AND YEAR(i.created_at) = :year";
    $count_query .= " AND YEAR(i.created_at) = :year";
...
// Get available years from the items
$stmt = $pdo->prepare("SELECT DISTINCT YEAR(created_at) as year FROM items WHERE location_id = ? ORDER BY year DESC");
$stmt->execute([$location_id]);
...
<div class="container-fluid" >
    ...
    ?> text-white">
        ...
        <div class="card-body">
            <?php if (!empty($search_filter) || ($month_filter && $month_filter !== 'all') || ($year_filter && $year_filter !== 'all')): ?>
