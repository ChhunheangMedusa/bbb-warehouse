...
// Get today's stock out history (deduct_qty)
$query = "SELECT 
    so.id,
    so.item_id,
    so.item_code, 
...
WHERE 
    DATE(so.action_at) = :today
    AND so.action_type = 'deduct'
ORDER BY so.action_at DESC";

$stmt = $pdo->prepare($query);
$stmt->bindParam(':today', $today);
$stmt->execute();
$stock_out_history = $stmt->fetchAll(PDO::FETCH_ASSOC);

$items_per_page = 10;
$total_items = count($stock_out_history);
$total_pages = ceil($total_items / $items_per_page);
$current_page = isset($_GET['page']) ? max(1, min($total_pages, intval($_GET['page']))) : 1;
$start_index = ($current_page - 1) * $items_per_page;
$paginated_items = array_slice($stock_out_history, $start_index, $items_per_page);
?>
...
<head>
    ...
    <title><?php echo t('todays_stock_out'); ?></title>
...
<body>
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4 no-print">
            <h2><?php echo t('todays_stock_out'); ?></h2>
            <div>
        ...
        <div class="print-only text-center mb-3">
            <h3><?php echo t('todays_stock_out'); ?></h3>
            <p><?php echo date('F j, Y'); ?></p>
        </div>
        
        <div class="card mb-4">
            <div class="card-header bg-danger text-white">
                <h5 class="mb-0"><?php echo t('todays_stock_out_history'); ?></h5>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php if (empty($paginated_items)): ?>
                                <tr>
                                    <td colspan="14" class="text-center"><?php echo t('no_stock_out_today'); ?></td>
                                </tr>
                            <?php else: ?>
